...
export class DeviceActivityPage extends React.Component<DeviceActivityPageProps, DeviceActivityPageState> {
    ...
    private signOutSession = (device: Device, session: Session) => {
      AccountServiceClient.Instance.doDelete("/sessions/" + session.id)
          .then (() => {
            this.fetchDevices();
            ContentAlert.success('Signed out ' + session.browser + '/' + device.os);
          });
    }
    ...
    private findDeviceName(device: Device): string {
      ...
      if (deviceName.includes('other') || deviceName.includes('generic') ) {
        return this.findOS(device) + ' ' + this.findOSVersion(device);
      }
                            ...
                            {device.sessions.map((session: Session, sessionIndex: number) => {
                              return (
                                <React.Fragment key={'device-' + deviceIndex + '-session-' + sessionIndex}>
                                  ...
                                  <GridItem span={9}>
                                    <p><strong>{session.browser} on</strong> {this.findOS(device)} {this.findOSVersion(device)}</p>
                                    {this.state.devices[0].mobile &&
                                      <p><strong>IP Address </strong> {session.ipAddress}</p>
                                    }
                                    <p><strong>Last accessed on</strong> {this.time(session.lastAccess)}</p>
                                    <p><strong>Clients</strong> {this.makeClientsString(session.clients)}</p>
                                    <p><strong>Started at</strong> {this.time(session.started)}</p>
                                    <p><strong>Expires at</strong> {this.time(session.expires)}</p>
                                    {!session.current && 
